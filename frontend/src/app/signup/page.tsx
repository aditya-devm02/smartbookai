    ...
    <>
      ...
      <main style={{ minHeight: '100vh', background: 'var(--background)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 8px' }}>
        <div style={{ background: 'var(--card-bg)', borderRadius: 16, boxShadow: '0 4px 24px #0008', padding: 'clamp(18px, 6vw, 40px)', maxWidth: 400, width: '100%', margin: 'auto', border: '2px solid var(--primary)' }}>
          <h2 style={{ color: 'var(--primary)', fontWeight: 900, fontSize: 'clamp(1.5rem, 5vw, 2rem)', marginBottom: 24, letterSpacing: 1 }}>Sign Up</h2>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            <input type="text" placeholder="Username" value={username} onChange={e => setUsername(e.target.value)} required style={{ width: '100%', marginBottom: 12, fontSize: '1rem', padding: '10px 12px' }} disabled={loading} />
            ...
            <button type="submit" style={{ width: '100%', marginTop: 8, fontSize: '1.1rem', fontWeight: 800, letterSpacing: 0.5, padding: '12px 0', borderRadius: 8 }} disabled={loading}>Sign Up</button>
          </form>
